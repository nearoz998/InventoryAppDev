...
@section('content')
    <div class="ibox">
        <div class="ibox-head">
            <div class="ibox-title">Edit Purchase</div>
                <div class="d-flex mx-auto my-2 btn-group">
                    ...
                    <a href="export-purchases" class="btn btn-primary my-auto py-auto px-auto ml-1"
                        onclick="return confirm('Click OK to export data as CSV file')"><i class="fa fa-upload"></i>&nbsp;Export CSV</a>
                </div>
        </div>
        <div class="ibox-body">
            <form class="form-horizontal" id="purchaseform-0" action="{{ route('purchases.update', $purchase->id) }}"
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col col-sm-7">
                                <input class="form-control" type="number" id="purchase_invoice_no-0" name="invoice_no"
                                    placeholder="Invoice No." value="{{ $purchase->invoice_no }}">
                            </div>
                        </div>
                    </div>
                    <div class="col">
                        <div class="form-group row">
                            ...
                            <div class="col col-sm-7">
                                <input class="form-control" type="date" id="purchase_date-0" name="date"
                                    placeholder="Date" value="{{ $purchase->date }}">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label class="col col-sm-2 col-form-label font-bold">Details</label>
                            <div class="col col-sm-7">
                                <textarea class="form-control" type="text" value="{{ $purchase->details }}"
                                    id="purchase_details-0" name="details" placeholder="details"></textarea>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col col-sm-3">
                        <select name="payment_type" id="purchase_payment_type-0" class="form-control select2_demo_2">
                            <option value="" selected disabled></option>
                            <option value="cash" {{ $purchase->payment_type == 'cash' ? 'selected' : '' }}>Cash Payment
                            </option>
                            <option value="bank" {{ $purchase->payment_type == 'bank' ? 'selected' : '' }}>Bank Payment
                            </option>
                            <option value="due" {{ $purchase->payment_type == 'due' ? 'selected' : '' }}>Due Payment
                            </option>
                        </select>
                    </div>
                </div>
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody id="dynamic-field">
                            @php
                                ...
                                $purchase_quantities = json_decode($purchase->quantity);
                                $purchase_prices = json_decode($purchase->price);
                                $purchase_sub_totals = json_decode($purchase->sub_total);
                            @endphp
                                ...
                                <tr id="rowpurchase_{{ $i }}-0">
                                        ...
                                        <select name="product[]" id="purchase_product{{ $i }}-0"
                                            ...
                                            @foreach ($products as $product)
                                                <option
                                                    ...
                                                    {{ $product_ids[$j] == $product->id ? 'selected' : '' }}>
                                                    {{ $product->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <td>
                                        <input type="number" name="stock[]" readonly
                                            id="purchase_stock{{ $i }}-0" class="form-control purchase_stock">
                                    </td>
                                    <td>
                                        <input type="number" name="quantity[]" min="0.01" step=".01"
                                            value="{{ $purchase_quantities[$j] }}" id="purchase_quantity{{ $i }}-0"
                                            class="form-control purchase_quantity">
                                    </td>
                                    <td>
                                        <input type="number" name="price[]" value="{{ $purchase_prices[$j] }}" readonly
                                            id="purchase_price{{ $i }}-0" class="form-control purchase_price">
                                    </td>
                                    <td>
                                        <input type="number" name="sub_total[]" value="{{ $purchase_sub_totals[$j] }}" readonly
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <td>
                                    <input type="number" name="total_amount" value="{{ $purchase->total }}" readonly
                                        id="purchase_total_amount-0" class="form-control purchase_total_amount">
                                </td>
                                <td>
        ...
        $(document).ready(function() {
            ...
            $('#purchase_add_more_item').click(function() {
                ...
                $('#dynamic-field').append(
                    ...
                    '<td><input type="number" name="price[]" readonly id="purchase_price' +
                    i +
    ...
    @include('cms.validations.formvalidation', ['id' => $purchase->id])
